...
.PHONY: db-up db-down migrate downgrade revision lint format typecheck test
...
lint:  ## ruff + black check
	cd backend && .venv/bin/ruff check app alembic && .venv/bin/black --check app alembic

format:  ## ruff --fix + black write
	cd backend && .venv/bin/ruff check app alembic --fix && .venv/bin/black app alembic
